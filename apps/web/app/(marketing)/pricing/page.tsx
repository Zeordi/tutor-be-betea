...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--muted)] dark:bg-[var(--background)]">
      <div className="mx-auto max-w-5xl px-6 py-16">
        <div className="mb-12 text-center">
          <h1 className="mb-3 text-4xl font-extrabold text-[var(--foreground)]">
            Simple, Transparent Pricing
          </h1>
          <p className="mb-6 text-[var(--muted-foreground)]">
            All plans include trust badges, safety tools, and local payment support
          </p>
          <div className="inline-flex rounded-xl bg-[var(--border)] p-1 dark:bg-[var(--card)]">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {PLANS.map((plan) => {
            ...
            return (
              <div
                key={plan.name}
                className={`relative rounded-3xl border-2 bg-[var(--card)] p-8 shadow-sm dark:bg-[var(--card)] ${
                  plan.popular
                    ? "border-[var(--primary)]"
                    : "border-[var(--border)]"
                }`}
              >
                ...
                <p className="mb-1 text-xl font-extrabold text-[var(--foreground)]">
                  {plan.name}
                </p>
                <p className="mb-4 text-xs text-[var(--muted-foreground)]">{plan.desc}</p>
                <p className="mb-1 text-4xl font-extrabold text-[var(--primary)]">
                ...
                <Link
                  ...
                  className={`block w-full rounded-xl py-3 text-center text-sm font-bold ${
                    plan.popular
                      ...
                      : "border-2 border-[var(--primary)] text-[var(--primary)] hover:bg-[var(--primary-light)]"
                  }`}
                >
